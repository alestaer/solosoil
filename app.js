...
// ===== TOCAR =====
async function tocarNotas(notas, reverbMix = 0.2) {
  await Tone.start();
  destruirSessao();
...
mapa.on('click', async (e) => {
  ...
  try {
    ...
    await tocarNotas(dados.notas, dados.musica.reverb_mix);
...
document.getElementById('btn-tocar').addEventListener('click', () => {
  if (ultimasNotas) {
    // Usa o último reverb conhecido (vai estar dentro da sessão actual)
    tocarNotas(ultimasNotas, sessao?.reverb?.wet?.value ?? 0.2);
  }
});
