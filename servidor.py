...
def parametros_musicais(v, diag, agua, osm, dif):
    env = DIFICULDADE[dif]
    ...
    # --- REGISTO / peso (densidade aparente = compactação) ---
    transpor = int(round(-(densidade - 1.3) * 6))        # mais compacto -> mais grave
...
# ============================================================
def progressao(tonal, n_compassos):
    if tonal.modo == "maior":
        base = [0, 4, 5, 3]          # I  V  vi IV
    else:
        base = [0, 5, 2, 4]          # i  VI III v  (escuro)
    prog = [base[c % len(base)] for c in range(n_compassos)]
    # cadência: último compasso de cada frase volta ao I/i
    return prog
...
# MELODIA
# ============================================================
def gerar_melodia(tonal, p, metro, n_compassos, oitava_base, lim_min, lim_max):
    ...
    for c in range(n_compassos):
        ...
        for i, dur_q in enumerate(celula):
            ...
            # grau -> MIDI
            midi = tonal.grau_para_midi(grau, oitava_base) + p["transpor"]

            # Cromatismo (nota fora da escala)
            alteracao_extra = 0
            if not (ultima_da_frase and i == n_ev - 1) and np.random.random() < p["prob_cromatico"]:
            ...
            prog = c / max(1, n_compassos - 1)
            arco = 1.0 - abs(prog - 0.5) * 0.5
...
def gerar_harmonia(tonal, p, metro, n_compassos, oitava_base, lim_min, lim_max, prog):
    ...
    for c in range(n_compassos):
        ...
        for add in (0, 2, 4):
            g = grau_raiz + add
            m = int(np.clip(tonal.grau_para_midi(g, oitava_base) + p["transpor"],
                            lim_min, lim_max))
...
def gerar_baixo(tonal, p, metro, n_compassos, oitava_base, lim_min, lim_max, prog):
    ...
    for c in range(n_compassos):
        ...
        if ultima or beats <= 2.0:
            padrao = [(grau_raiz, d) for d in _blocos_sustentado(beats)]
        elif beats == 3.0:
            padrao = [(grau_raiz, 1.0), (grau_raiz + 4, 1.0), (grau_raiz, 1.0)]
        elif beats == 4.0:
            padrao = [(grau_raiz, 1.0), (grau_raiz + 4, 1.0),
                      (grau_raiz, 1.0), (grau_raiz + 4, 1.0)]
        elif beats == 5.0:
            padrao = [(grau_raiz, 2.0), (grau_raiz + 4, 2.0), (grau_raiz, 1.0)]
        else:  # 3.5
            padrao = [(grau_raiz, 1.0), (grau_raiz + 4, 1.0), (grau_raiz, 1.0),
                      (grau_raiz + 4, 0.5)]
        for g, dur_q in padrao:
            ...
            eventos.append({
                ...
                "inicio_seg": round(tempo_q * seg_por_q, 4),
                "duracao_seg": round(dur_q * 0.92 * seg_por_q, 4),
            })
            tempo_q += dur_q
        compassos.append(eventos)
    return compassos
...
def _mxml_nota(ev, voz, staff, num_string=None):
    dur = int(round(ev["dur_q"] * DIVISIONS))
    tipo = MXML_TYPE[ev["dur_q"]]
    out = ["      <note>"]
    if ev["is_rest"]:
        out.append("        <rest/>")
        ...
        return "\n".join(out)
    # nota principal
    def pitch_xml(step, alter, octave, indent="        "):
        ...
        return "\n".join(s)
    out.append(pitch_xml(ev["step"], ev["alter"], ev["octave"]))
    out.append(f"        <duration>{dur}</duration>")
    out.append(f"        <voice>{voz}</voice>")
    out.append(f"        <type>{tipo}</type>")
    if ev.get("staccato"):
    ...
    for n in ev.get("acorde", []):
        c = ["      <note>", "        <chord/>"]
        c.append(pitch_xml(n["step"], n["alter"], n["octave"]))
...
def construir_musicxml(partes, tonal, metro, titulo="CuriouSoil"):
    ...
    for idx, parte in enumerate(partes):
        pid = f"P{idx+1}"
        nome = INSTRUMENTOS.get(parte["instrumento"], {}).get("nome", parte["papel"])
        gm = INSTRUMENTOS.get(parte["instrumento"], {}).get("gm", 0)
        L.append(f'    <score-part id="{pid}">')
        L.append(f'      <part-name>{xml_escape(nome)} ({parte["papel"]})</part-name>')
    ...
    for idx, parte in enumerate(partes):
        pid = f"P{idx+1}"
        clef_sign, clef_line = clef_map[parte["clef"]]
        L.append(f'  <part id="{pid}">')
        for m_idx, compasso in enumerate(parte["compassos"]):
            L.append(f'    <measure number="{m_idx+1}">')
            if m_idx == 0:
                L.append('      <attributes>')
                L.append(f'        <divisions>{DIVISIONS}</divisions>')
                L.append(f'        <key><fifths>{fifths}</fifths>'
                         f'<mode>{modo if modo=="major" else ("major" if modo=="maior" else "minor")}</mode></key>')
                L.append(f'        <time><beats>{num}</beats>'
                         f'<beat-type>{den}</beat-type></time>')
                L.append(f'        <clef><sign>{clef_sign}</sign>'
                         f'<line>{clef_line}</line></clef>')
                L.append('      </attributes>')
            for ev in compasso:
                L.append(_mxml_nota(ev, voz=1, staff=1))
            L.append('    </measure>')
        L.append('  </part>')
...
def montar_peca(valores, agua, diag, osm, opcoes):
    ...
    p = parametros_musicais(valores, diag, agua, osm, dif)
    p["_dif"] = dif
    ...
    # arredonda a múltiplo do tamanho da frase para fechar cadências
    ...
    prog = progressao(tonal, n_compassos)
    papeis = atribuir_papeis(instrumentos)

    partes = []
    for papel, instr_id in papeis:
        oitava_base, lim_min, lim_max = registo_para_papel(
            papel, instr_id, p["transpor"], tonal.tonic_pc)
        if papel == "melodia":
            comps = gerar_melodia(tonal, p, metro, n_compassos, oitava_base,
                                  lim_min, lim_max)
        elif papel == "harmonia":
            comps = gerar_harmonia(tonal, p, metro, n_compassos, oitava_base,
                                   lim_min, lim_max, prog)
        else:
            comps = gerar_baixo(tonal, p, metro, n_compassos, oitava_base,
                                lim_min, lim_max, prog)
        partes.append({
            "papel": papel,
